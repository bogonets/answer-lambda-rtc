...
class VideoImageTrack(VideoStreamTrack):
    def __init__(self, queue):
        ...
        self.queue = queue
        print_error(f'rtc.realtime_video.VideoImageTrack.recv() __init__')

    async def recv(self):
        print_error(f'rtc.realtime_video.VideoImageTrack.recv() BEGIN')
        ...
        try:
            ...
            img = None
            print_error(f'rtc.realtime_video.VideoImageTrack.recv() Empty queue error.')
        ...
        if img is not None:
            try:
                ...
                print_error(f'rtc.realtime_video.VideoImageTrack.recv() Image to ndarray error.')
                frame = last_frame
        else:
            frame = last_frame
        ...
        print_out(f'rtc.realtime_video.VideoImageTrack.recv() frame: {last_frame}')
        return frame
...
async def on_exit_process_background():
    ...
    consumer_queue.cancel_join_thread()
...
def on_init():
    ...
    print_out(f'RTC process PID is {rtc_process.pid}.')
...
def on_destroy():
    ...
    if rtc_process.is_alive():
        print_error('Terminate the RTC process.')
        rtc_process.kill()

    # A negative value -N indicates that the child was terminated by signal N.
    print_error(f'The exit code of RTC process is {rtc_process.exitcode}.')
    rtc_process.close()
    print_error('rtc_process closed.')

    global producer_queue
    producer_queue.close()
    producer_queue.cancel_join_thread()
